...
const sendErrorProd = (err, req, res) => {
    // A) API
    if (req.originalUrl.startsWith('/api')) {
        // A) We set this error as operational (an issue with something like an input from client), so we want the client to know what
        // went wrong
    ...
    // B) The error is something else that we don't want to send too much info to the client about because it's unrelated to the client
    // 1) Log Error
    console.error('ERROR 💥', err);
    // 2) Send generic message
    // RENDERED WEBSITE
